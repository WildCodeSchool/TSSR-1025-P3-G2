...
- [Configuration du Serveur Bastion - Apache Guacamole](#configuration-du-serveur-bastion---apache-guacamole)
  ...
  - [6. Configuration du Reverse Proxy HTTPS avec Nginx](#6-configuration-du-reverse-proxy-https-avec-nginx)
    - [6.1. Contexte et choix architectural](#61-contexte-et-choix-architectural)
      - [Problème initial : HAProxy sur pfSense](#problème-initial--haproxy-sur-pfsense)
      - [Solution retenue : Nginx en conteneur Docker](#solution-retenue--nginx-en-conteneur-docker)
    ...
    - [6.4. Modification de la stack Docker](#64-modification-de-la-stack-docker)
      - [Ajout du service Nginx](#ajout-du-service-nginx)
  ...
  - [7. Configuration DNS](#7-configuration-dns)
    - [7.1. Création de l'enregistrement DNS](#71-création-de-lenregistrement-dns)
    - [7.2. Validation](#72-validation)
  - [7. Configuration des règles de pare-feu pfSense](#7-configuration-des-règles-de-pare-feu-pfsense)
    ...
    - [7.3. Règles sur l'interface BASTION (flux sortants)](#73-règles-sur-linterface-bastion-flux-sortants)
      - [7.3.1. Suppression de la règle temporaire](#731-suppression-de-la-règle-temporaire)
      - [7.3.2. Règle 1 : Résolution DNS](#732-règle-1--résolution-dns)
      - [7.3.3. Règle 2 : Synchronisation horaire NTP](#733-règle-2--synchronisation-horaire-ntp)
      - [7.3.4. Règle 3 : Mises à jour système et Docker](#734-règle-3--mises-à-jour-système-et-docker)
      - [7.3.5. Règle 4 : Connexions RDP vers serveurs Windows](#735-règle-4--connexions-rdp-vers-serveurs-windows)
      - [7.3.6. Règle 5 : Connexions SSH vers serveurs Linux](#736-règle-5--connexions-ssh-vers-serveurs-linux)
      - [7.3.7. Ordre des règles](#737-ordre-des-règles)
    - [7.4. Règles sur l'interface ADMIN (flux entrants)](#74-règles-sur-linterface-admin-flux-entrants)
      - [7.4.1. Règle d'accès HTTPS au bastion](#741-règle-daccès-https-au-bastion)
    - [7.5. Création d'alias (bonne pratique)](#75-création-dalias-bonne-pratique)
      - [7.5.1. Alias pour le bastion](#751-alias-pour-le-bastion)
      - [7.5.2. Alias pour les serveurs Windows](#752-alias-pour-les-serveurs-windows)
      - [7.5.3. Alias pour les serveurs Linux](#753-alias-pour-les-serveurs-linux)
    - [7.6. Tests de validation](#76-tests-de-validation)
      - [Test 1 : Accès web au bastion depuis le VLAN Admin](#test-1--accès-web-au-bastion-depuis-le-vlan-admin)
      - [Test 2 : Connexions RDP/SSH depuis le bastion](#test-2--connexions-rdpssh-depuis-le-bastion)
      - [Test 3 : Mises à jour Docker](#test-3--mises-à-jour-docker)
      - [Test 4 : Vérification des logs pfSense](#test-4--vérification-des-logs-pfsense)
    - [7.7. Matrice récapitulative des règles](#77-matrice-récapitulative-des-règles)
      - [Règles sur l'interface BASTION (sortant du bastion)](#règles-sur-linterface-bastion-sortant-du-bastion)
      - [Règles sur l'interface ADMIN (accès au bastion)](#règles-sur-linterface-admin-accès-au-bastion)
    ...
    - [7.9. Considérations de sécurité](#79-considérations-de-sécurité)
      - [Principe du moindre privilège appliqué](#principe-du-moindre-privilège-appliqué)
      - [Recommandations pour durcissement supplémentaire](#recommandations-pour-durcissement-supplémentaire)
